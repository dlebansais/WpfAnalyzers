...
public partial class ValidCalls
{
    ...
    {
        CheckContext checkContext = new() { AssertNoCaller = true, AssertBadCaller = true, AssertUnknownCaller = true };
        await Helper.CheckSolutionAsync("Valid-OneObservableCollectionAfterConfigureAwait", checkContext);
    }

    [Test]
    public async Task AppOneNamedControlNestedConfigureAwait()
    {
        CheckContext checkContext = new() { AssertNoCaller = true, AssertBadCaller = true, AssertUnknownCaller = true };
        await Helper.CheckSolutionAsync("Valid-OneNamedControlNestedConfigureAwait", checkContext);
    }
}
